...
class SparqlInterface:
    ...
    def export_rdf (self, full_export=False):
        ...
        #root_types = ["datasets", "collections", "accounts"]
        root_types = ["Account", "Collection", "Dataset", "Review", "InstitutionGroup", "PrivateLink"]
        for record_type in root_types:
            query = self.__query_from_template (f"export_root_type", {
                "full_export": full_export,
                "type_name": record_type
            })
            self.__export_rdf_to_file (query, export_directory, f"{record_type}.n3")

        listable_types = ["categories", "authors", "files", "funding_list",
                          "references", "tags", "datasets", "private_links", "collaborators"]
        for record_type in listable_types:
            query = self.__query_from_template (f"export_listable_type", {
                "full_export": full_export,
