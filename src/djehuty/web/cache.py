...
class CacheLayer:
    ...
    def cached_value (self, prefix, key):
        ...
        data = None
        try:
            filename = os.path.join (f"{self.storage}", f"{prefix}_{key}")
    ...
    def cache_value (self, prefix, key, value, query=None):
        ...
        try:
            cache_filename = os.path.join (f"{self.storage}", f"{prefix}_{key}")
            ...
            if query is not None:
                query_filename = os.path.join (f"{self.storage}", f"{prefix}_{key}.sparql")
    ...
    def remove_cached_value (self, prefix, key):
        ...
        file_path = os.path.join (f"{self.storage}", f"{prefix}_{key}")
        try:
            os.remove(file_path)
    ...
    def invalidate_by_prefix (self, prefix):
        ...
        for file_path in glob.glob(os.path.join(f"{self.storage}", f"{prefix}_*")):
            try:
                os.remove(file_path)
            except FileNotFoundError:
                self.log.warning ("Trying to remove %s multiple times.", file_path)
    ...
    def invalidate_all (self):
        ...
        files = glob.glob(os.path.join(f"{self.storage}", "*"))
